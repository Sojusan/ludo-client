...
class GameBoard : VerticalLayout {
    ...
    void movePlayerPawn(string from, string to) {
        ...
        foreach (HorizontalLayout child; this._children) {
            Log.i(child.id);
            auto board_piece = child.childById(from);
            if (!is(typeof(board_piece is null))) {
                Log.i(board_piece.id);
            } else {
                Log.i("Null");
            }
            foreach (Widget child; child._children) {

    ...
    private string process_message(JSONValue server_response){
        ...
        switch(findCommandId(to!string(server_response["action"]))) {
            ...
            case LudoCommand.NEW_PLAYER:
                ...
                _gamePage._mainMenu.setTotalPlayers(to!int(to!string(server_response["totalPlayers"])));
                _gamePage._mainMenu.setReadyPlayers(to!int(to!string(server_response["totalPlayers"])));

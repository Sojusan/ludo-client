...
class GameBoard : VerticalLayout {
    ...
    ///
    void movePlayerPawn(string pawn, string from, string to) {
        ...
        if (from_field.childCount > 1) {
            foreach (child_id; 1 .. from_field.childCount) {
                ImageWidget child = from_field.childById!ImageWidget(from_field.child(child_id).id);
...
class GameWidget : FrameLayout, LudoClientCallback, BoardPiecesCallback {
    ...
    void onBoardPieceClick(string buttonId) {
        ...
        if (currentPlayer != _nickname) {
            ...
            if (_gamePage._menu._diceResult.text() != "0") {
                JSONValue data_to_send = ["action": "pawn_clicked"];
                data_to_send.object["player"] = JSONValue(_nickname);
                data_to_send.object["field"] = JSONValue(buttonId);
    ...
    private string process_message(JSONValue server_response){
        ...
        switch(findCommandId(to!string(server_response["action"]))) {
            ...
            case LudoCommand.NEXT_TURN:
                ...
                _gamePage._menu.setDiceResult(to!int(0));
                break;

            case LudoCommand.MOVE_PLAYER_PAWN:
                message = null;
                string pawn_to_move = strip(to!string(server_response["pawn_to_move"]), "\"");
                string source_field = strip(to!string(server_response["source_field"]), "\"");
                string destination_field = strip(to!string(server_response["destination_field"]), "\"");
                string to_delete_pawns = strip(to!string(server_response["to_delete_pawns"]), "\"");
                deletePawns(destination_field, to_delete_pawns, _gamePage);
                _gamePage._gameBoard.movePlayerPawn(pawn_to_move, source_field, destination_field);
                _gamePage._menu.setDiceResult(to!int(0));
                break;

            case LudoCommand.PLAYER_WIN:
                string player = strip(to!string(server_response["player"]), "\"");
                message = "Player: " ~ player ~ " won a game!";

                _gamePage._menu.setCurrentPlayer("None"d);
                _gamePage._menu.setDiceResult(0);
                _gamePage._menu.setPlayer("None"d);
                _gamePage._menu._rollButton.enabled = false;
                _gamePage._menu._endTurnButton.enabled = false;
                _gamePage._gameBoard.deleteAllPawns();
                _gamePage._mainMenu.setReadyPlayers(0);
                _gamePage._mainMenu._readyButton.enabled = true;
    ...
    private LudoCommand findCommandId(string command) {
        ...
        switch (strip(command, "\"")) with(LudoCommand) {
            ...
            case "move_player_pawn": return MOVE_PLAYER_PAWN;
    ...
    /// Delete pawns
    private void deletePawns(string source_field, string to_delete_pawns, ref GamePage gamePage) {
        if (canFind(to_delete_pawns, "yellow_pawn_1")) {
            gamePage._gameBoard.movePlayerPawn("yellow_pawn_1", source_field, "yellow_base_1");
        }
        if (canFind(to_delete_pawns, "yellow_pawn_2")) {
            gamePage._gameBoard.movePlayerPawn("yellow_pawn_2", source_field, "yellow_base_2");
        }
        if (canFind(to_delete_pawns, "yellow_pawn_3")) {
            gamePage._gameBoard.movePlayerPawn("yellow_pawn_3", source_field, "yellow_base_3");
        }
        if (canFind(to_delete_pawns, "yellow_pawn_4")) {
            gamePage._gameBoard.movePlayerPawn("yellow_pawn_4", source_field, "yellow_base_4");
        }
        if (canFind(to_delete_pawns, "green_pawn_1")) {
            gamePage._gameBoard.movePlayerPawn("green_pawn_1", source_field, "green_base_1");
        }
        if (canFind(to_delete_pawns, "green_pawn_2")) {
            gamePage._gameBoard.movePlayerPawn("green_pawn_2", source_field, "green_base_2");
        }
        if (canFind(to_delete_pawns, "green_pawn_3")) {
            gamePage._gameBoard.movePlayerPawn("green_pawn_3", source_field, "green_base_3");
        }
        if (canFind(to_delete_pawns, "green_pawn_4")) {
            gamePage._gameBoard.movePlayerPawn("green_pawn_4", source_field, "green_base_4");
        }
        if (canFind(to_delete_pawns, "blue_pawn_1")) {
            gamePage._gameBoard.movePlayerPawn("blue_pawn_1", source_field, "blue_base_1");
        }
        if (canFind(to_delete_pawns, "blue_pawn_2")) {
            gamePage._gameBoard.movePlayerPawn("blue_pawn_2", source_field, "blue_base_2");
        }
        if (canFind(to_delete_pawns, "blue_pawn_3")) {
            gamePage._gameBoard.movePlayerPawn("blue_pawn_3", source_field, "blue_base_3");
        }
        if (canFind(to_delete_pawns, "blue_pawn_4")) {
            gamePage._gameBoard.movePlayerPawn("blue_pawn_4", source_field, "blue_base_4");
        }
        if (canFind(to_delete_pawns, "red_pawn_1")) {
            gamePage._gameBoard.movePlayerPawn("red_pawn_1", source_field, "red_base_1");
        }
        if (canFind(to_delete_pawns, "red_pawn_2")) {
            gamePage._gameBoard.movePlayerPawn("red_pawn_2", source_field, "red_base_2");
        }
        if (canFind(to_delete_pawns, "red_pawn_3")) {
            gamePage._gameBoard.movePlayerPawn("red_pawn_3", source_field, "red_base_3");
        }
        if (canFind(to_delete_pawns, "red_pawn_4")) {
            gamePage._gameBoard.movePlayerPawn("red_pawn_4", source_field, "red_base_4");
        }
    }
}

...
LudoCommand findCommandId(string command) {
    ...
    switch (command) with(LudoCommand) {
        case "error"d: return ERROR;
        case "new_player"d: return NEW_PLAYER;
...
string process_message(JSONValue server_response){
    ...
    const auto action = findCommandId(to!string(server_response["action"]));
    Log.i("Action found: " ~ to!string(action));
    switch(action) {

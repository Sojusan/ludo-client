...
class GameBoard : VerticalLayout {
    ...
    void movePlayerPawn(string from, string to) {
        ...
        if (from_field.childCount > 1) {
            ImageWidget child = from_field.childById!ImageWidget;
            Log.i("TEST: " ~ "id: " ~ from_field.child(1).id ~ " image: " ~ from_field.child(1)._drawableId);
            this.addPawn(to_field, from_field.child(1).id, from_field.child(1).backgroundImageId);
